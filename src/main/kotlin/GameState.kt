...
data class GameState(
    ...
    val selectedHexes: Set<Hex> = setOf(),
...
//        }
        ...
        val hexAtClick : Hex? = hexMap.getHexAtClick(actionEvent)
        val residentEntity = hexMap.getEntityAtClick(actionEvent)

        // toggle hexes
        return if (hexAtClick != null) {

            if (!selectedHexes.contains(hexAtClick)) {
                println("> 1")
                this.copy(selectedHexes = selectedHexes.plus(hexAtClick))
            } else {
                println("> 2")
                this.copy(selectedHexes = selectedHexes.minus(hexAtClick))
            }
        } else {
            println("> 3")
            this
